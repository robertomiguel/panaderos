...
$tmp = Formatos::primervencimiento($boletasindical->getPeriodo());
 echo $tmp; ?>',
   maxDate: '<?php
$dia = substr($tmp,0,2)*1;
$mes = substr($tmp,3,2)*1;
$anio = substr($tmp,6,4)*1;
$mes = $mes + 3;
if ($mes=="13"){$anio++;$mes="01";}
if ($mes=="14"){$anio++;$mes="02";}
if ($mes=="15"){$anio++;$mes="03";}
  $mes = str_pad($mes, 2, "0", STR_PAD_LEFT);
 echo "$dia/$mes/$anio";
if ("$anio$mes$dia"<date('Ymd')){$pasadosindical=1;}
?>',showOn: 'both',buttonText: 'Cambiar',autoSize: true,
//   changeYear: true
...
if ($mes=="15"){$anio++;$mes="03";}
 echo "$dia/$mes/$anio";
  $mes = str_pad($mes, 2, "0", STR_PAD_LEFT);
if ("$anio$mes$dia"<date('Ymd')){$pasadofondo=1;}
?>',showOn: 'both',buttonText: 'Cambiar',autoSize: true,
//   changeYear: true
   beforeShowDay: function (day) {var day = day.getDay();
if (day == 6 || day == 0) {return [false, "somecssclass"]
...
ETIQUETA;
if ($pasadosindical) {echo $atrasado;} else {
...
<?php
if ($pasadofondo) {echo $atrasado;} else {
